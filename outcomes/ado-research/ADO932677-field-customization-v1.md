...
LinkCentral API (REST)
  ...
  ↓
GeneralLinkService.getLineupItemsFromKiwiplan()
  ↓
...
1. **GeneralLinkService** (`linkdevice-app/src/main/java/com/kiwiplan/linkdevice/link/GeneralLinkService.java`)
   - Contains `getLineupItemsFromKiwiplan()` method
...
links:
  - id: gopfert_link_1
    ...
    configurations:
      ...
      # NEW SECTION - Field Customization
      fieldCustomization:
        enabled: true
        rules:
          # Rule 1: Simple field override with percentage increase
          - fieldPath: "productSpecification.board.length"
            overridable: true
            expression: "item.productSpecification.board.length * 1.1"
            description: "Add 10% buffer to length for manufacturing tolerance"
            
          # Rule 2: Conditional override based on customer
          - fieldPath: "unitising.piecesRequired"
            overridable: true
            expression: "item.customerName == 'ACME' ? item.unitising.piecesRequired + 5 : item.unitising.piecesRequired"
            description: "Add 5 pieces buffer for ACME customer orders"
            
          # Rule 3: Derived field calculation
          - fieldPath: "productSpecification.customField1"
            overridable: true
            expression: "item.productSpecification.board.width + item.productSpecification.board.length"
            description: "Calculate total dimension (width + length)"
            
          # Rule 4: Complex conditional with multiple conditions
          - fieldPath: "unitising.piecesRequired"
            overridable: true
            expression: |
              item.unitising.piecesRequired > 100 && item.customerName == 'PRIORITY' 
                ? item.unitising.piecesRequired * 1.05 
                : item.unitising.piecesRequired
            description: "Apply 5% buffer for large priority customer orders"
            
          # Rule 5: String manipulation
          - fieldPath: "orderNumber"
            overridable: true
            expression: "'CUSTOM-' + item.orderNumber"
            description: "Prefix order number with CUSTOM"
...
controllers:
  - id: gopfert_controller_1
    description: "Gopfert Converting Machine Controller"
...
#### 1. FieldCustomizationConfig.java
...
/**
 * Individual field customization rule.
 * Defines a JEXL expression to transform a specific field in a LineupItemDto.
 */
@JsonIgnoreProperties(ignoreUnknown = false)
public class FieldCustomizationRule {
    
    @NotBlank(message = "Field path is required and cannot be blank")
    ...
    @NotBlank(message = "JEXL expression is required and cannot be blank")
    ...
    /**
     * Get the field path (dot-notation) for the field to customize.
     * Examples: "productSpecification.board.length", "unitising.piecesRequired"
     */
    ...
    /**
     * Check if this field is overridable.
     * If false, the rule will not be applied even if it exists.
     */
    ...
    /**
     * Get the JEXL expression to evaluate.
     * The expression has access to the full LineupItemDto via the 'item' variable.
     * Example: "item.productSpecification.board.length * 1.1"
     */
    ...
    /**
     * Get optional description for documentation purposes.
     */
...
/**
 * Service for applying field customizations using JEXL expressions.
 * 
 ...
 * 
 * Key Features:
 * - JEXL expression evaluation with full access to LineupItemDto
 * - Nested field path support (dot notation)
 * - Safe fallback: returns original value on any error
 * - Respects overridable flag on rules
 * 
...
public class FieldCustomizationService {
        ...
        try {
            ...
            for (FieldCustomizationRule rule : rules) {
                if (rule.isOverridable()) {
                    boolean applied = applyRule(itemMap, rule, item);
                    if (applied) {
                        appliedCount++;
                    } else {
                        skippedCount++;
                    ...
                    log.trace("Skipping non-overridable rule for field: {}", rule.getFieldPath());
                    skippedCount++;
                }
            }
     ...
     * @param originalItem The original item (for JEXL context)
     * @return true if rule was applied, false if skipped/failed
     */
    private boolean applyRule(Map<String, Object> itemMap, FieldCustomizationRule rule, Object originalItem) {
        try {
            log.trace("Applying rule for field: {} with expression: {}", 
                rule.getFieldPath(), rule.getExpression());
            
            // Get original value for logging
            Object oldValue = getFieldValue(itemMap, rule.getFieldPath());
            
            // Create JEXL context with the item
            JexlContext context = new MapContext();
            context.set("item", originalItem);
            
            // Evaluate expression
            JexlExpression expression = jexlEngine.createExpression(rule.getExpression());
            Object result = expression.evaluate(context);
            
            // Apply result to field path
            if (result != null) {
                setFieldValue(itemMap, rule.getFieldPath(), result);
...
The single integration point is in the `getLineupItemsFromKiwiplan()` method:
...
public class GeneralLinkService extends LinkService {
    ...
    public GeneralLinkService(AbstractLinkConfiguration linkConfig, 
                              ...
                              FieldCustomizationService fieldCustomizationService) {  // NEW PARAMETER
        ...
        this.fieldCustomizationService = fieldCustomizationService;  // NEW ASSIGNMENT
     ...
     */
    private List<LineupItemDto> getLineupItemsFromKiwiplan() {
        ...
        log.debug("Fetching lineup from Link Central for machine {} (limit: {})", machineId, runListSize);
        
        // Fetch data from Link Central API
        LineupDto lineupDto = this.linkcentralClient.getLineupEntries(machineId, runListSize);
        ...
        FieldCustomizationConfig customizationConfig = getLinkConfig()
            .getConfigurations()
            .getFieldCustomization();
            
        if (customizationConfig != null && customizationConfig.isEnabled()) {
            log.info("Applying field customizations to {} lineup items", lineupItems.size());
            
            try {
                lineupItems = lineupItems.stream()
                    .map(item -> fieldCustomizationService.applyCustomizations(
                        item, 
                        customizationConfig.getRules()
                    ))
            ...
            log.debug("Field customization is disabled or not configured");
        }
...
Update all factory methods to inject the new service:
...
public class LinkServiceFactory {
    ...
    private GeneralLinkService getDownloadOnlyConverterLinkService(
        ...
        FieldCustomizationService fieldCustomizationService = 
            applicationContext.getBean(FieldCustomizationService.class);  // NEW
        ...
        return new DownloadOnlyConverterLinkService(
            ...
            controllerService, 
            nitriteRepo,
            fieldCustomizationService  // NEW PARAMETER
        );
    }
...
public class DownloadOnlyConverterLinkService extends GeneralLinkService {
    ...
    public DownloadOnlyConverterLinkService(
        ...
        super(linkConfig, linkCentralClient, schedulerService, controllerService, 
              nitriteRepo, fieldCustomizationService);  // PASS TO PARENT
    }
...
public class CorrugatorLinkService extends LinkService {
    ...
    private final SchedulerService schedulerService;
    private final NitriteRepo nitriteRepo;
    private final FieldCustomizationService fieldCustomizationService;  // NEW
    ...
    // ... rest of class (add field customization in getLineupItemsFromKiwiplan if exists) ...
}
```
...
class FieldCustomizationServiceTest {
    ...
    void testSimpleNumericFieldCustomization() {
        ...
        rule.setFieldPath("productSpecification.board.length");
        rule.setExpression("item.productSpecification.board.length * 1.1");
    ...
    @Test
    @DisplayName("Should apply conditional field customization")
    void testConditionalFieldCustomization() {
        // Given
        LineupItemDto item = createTestItem();
        item.setCustomerName("ACME");
        item.getUnitising().setPiecesRequired(100);
        
        FieldCustomizationRule rule = new FieldCustomizationRule();
        rule.setFieldPath("unitising.piecesRequired");
        rule.setExpression("item.customerName == 'ACME' ? item.unitising.piecesRequired + 5 : item.unitising.piecesRequired");
        rule.setOverridable(true);
        ...
        // Then
        assertEquals(105, result.getUnitising().getPiecesRequired());
    }
    ...
    void testReturnsOriginalValueOnExpressionError() {
        ...
        FieldCustomizationRule rule = new FieldCustomizationRule();
        rule.setFieldPath("productSpecification.board.length");
        rule.setExpression("item.nonExistentField * 2"); // Invalid expression
        rule.setOverridable(true);
    ...
    @Test
    @DisplayName("Should skip non-overridable rules")
    void testSkipsNonOverridableRules() {
        // Given
        LineupItemDto item = createTestItem();
        item.getProductSpecification().getBoard().setLength(100.0);
        ...
        FieldCustomizationRule rule = new FieldCustomizationRule();
        rule.setFieldPath("productSpecification.board.length");
        rule.setExpression("item.productSpecification.board.length * 2");
        rule.setOverridable(false); // Rule is disabled
    ...
    void testMultipleRulesInSequence() {
        ...
        rule1.setFieldPath("productSpecification.board.length");
        rule1.setExpression("item.productSpecification.board.length * 1.1");
        rule1.setOverridable(true);
        
        FieldCustomizationRule rule2 = new FieldCustomizationRule();
        rule2.setFieldPath("productSpecification.board.width");
        rule2.setExpression("item.productSpecification.board.width * 1.2");
        rule2.setOverridable(true);
...
- [ ] Performance is acceptable (no significant delay)
- [ ] Logs show clear information about applied rules
- [ ] Errors are logged but don't break the link
...
# Links Configuration
links:
  # Gopfert Converting Link with Field Customization
  - id: gopfert_converter_link
    description: "Gopfert Converting Machine Link with Custom Field Rules"
    type: converter
    subType: null
    ...
    configurations:
      ...
      trialMode: false          # Send actual messages to controller
      
      # Kiwiplan Connection
      kiwiplan:
        machineId: "1101"       # Machine ID in Kiwiplan
        lineupSize: 5           # Maximum lineup items to fetch
      
      # Field Customization Configuration
      fieldCustomization:
        enabled: true
        rules:
          # Rule 1: Manufacturing Tolerance
          - fieldPath: "productSpecification.board.length"
            overridable: true
            expression: "item.productSpecification.board.length * 1.05"
            description: "Add 5% manufacturing tolerance to board length"
          
          # Rule 2: Manufacturing Tolerance for Width
          - fieldPath: "productSpecification.board.width"
            overridable: true
            expression: "item.productSpecification.board.width * 1.05"
            description: "Add 5% manufacturing tolerance to board width"
          
          # Rule 3: Priority Customer Buffer
          - fieldPath: "unitising.piecesRequired"
            overridable: true
            expression: |
              item.customerName == 'ACME_CORP' || item.customerName == 'PRIORITY_CLIENT' 
                ? item.unitising.piecesRequired + 10 
                : item.unitising.piecesRequired
            description: "Add 10 pieces buffer for priority customers"
          
          # Rule 4: Large Order Adjustment
          - fieldPath: "unitising.piecesRequired"
            overridable: true
            expression: |
              item.unitising.piecesRequired > 1000 
                ? item.unitising.piecesRequired * 1.02 
                : item.unitising.piecesRequired
            description: "Add 2% buffer for large orders (>1000 pieces)"
          
          # Rule 5: Custom Order Number Prefix
          - fieldPath: "orderNumber"
            overridable: true
            expression: "'FAC-' + item.orderNumber"
            description: "Add factory prefix to order number"
          
          # Rule 6: Derived Field - Total Dimension
          - fieldPath: "productSpecification.customDimension"
            overridable: true
            expression: |
              item.productSpecification.board.length + 
              item.productSpecification.board.width
            description: "Calculate combined dimension for validation"
...
After field customization:
```json
{
  "orderNumber": "FAC-12345",
  "customerName": "ACME_CORP",
  "productSpecification": {
    "board": {
      "length": 1050.0,        // 1000 * 1.05
      "width": 525.0           // 500 * 1.05
    },
    "customDimension": 1575.0  // 1050 + 525
  },
  "unitising": {
    "piecesRequired": 110      // 100 + 10 (priority customer)
  }
}
```
...
✅ **Flexible Expressions**: Powerful JEXL syntax for complex logic  
...
✅ **Gradual Rollout**: Can enable/disable per link  
...
- [ ] Create `FieldCustomizationConfig.java`
- [ ] Create `FieldCustomizationRule.java`
- [ ] Update `LinkConfigurations.java` with new field
- [ ] Create `FieldCustomizationService.java`
- [ ] Update `GeneralLinkService.java` constructor and method
- [ ] Update `LinkServiceFactory.java` to inject service
- [ ] Update `DownloadOnlyConverterLinkService.java` constructor
- [ ] Update `ConveyorLinkService.java` constructor
- [ ] Update `CorrugatorLinkService.java` if applicable
